...
class TestUtils:
    """Class contains tests for utils.py"""

    @pytest.mark.parametrize(
        "tip_included,expected_output", [(False, 0.0), (True, 5.0)]
    )
    def test_tip(self, mocker, tip_included, expected_output):
        """Test tip amount."""
    ...
    def test_authenticate_user(self, db_session, mocker):
    ...
    def test_create_access_token(self, mocker, expires_delta, expected_output):
        """Test create access token."""
        mocker.patch("jose.jwt.encode", return_value="encoded")
        result = create_access_token(
            data={"sub": "string"}, expires_delta=expires_delta
        )
        assert result == expected_output
